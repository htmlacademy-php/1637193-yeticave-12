...
<section class="lot-item container">
    <h2><?= htmlspecialchars($lot['title'] ?? 'Без названия', ENT_QUOTES | ENT_HTML5) ?></h2>
            ...
            <div class="lot-item__state">
                ...
                <div class="lot-item__cost-state">
                    <div class="lot-item__rate">
                        ...
                        <span
                            class="lot-item__cost"><?= htmlspecialchars(formatted_sum($lot['current_price']),
                                ENT_QUOTES | ENT_HTML5) ?? 0 ?></span>
                    </div>
                    <div class="lot-item__min-cost">
                        Мин. ставка
                        <span><?= htmlspecialchars(formatted_sum($lot['current_price'] + $lot['bet_step']),
                                ENT_QUOTES | ENT_HTML5) ?? 0 ?></span>
                    </div>
                </div>
                <?php if ($show_bet_add) : ?>
                    <form class="lot-item__form" action="/lot.php?id=<?= htmlspecialchars($item_id ?? 0,
                        ...
                        <p class="lot-item__form-item <?= isset($errors['cost']) ? 'form__item form__item--invalid' : '' ?>">
                            ...
                            <input id="cost" type="text" name="cost"
                                   placeholder="<?= htmlspecialchars(formatted_sum($lot['current_price'] + $lot['bet_step']),
                                       ENT_QUOTES | ENT_HTML5) ?? 0 ?>"
                                   value="<?= htmlspecialchars($cost, ENT_QUOTES | ENT_HTML5) ?>">
                    ...
                    <?php if (isset($bets)): ?>
                        <?php foreach ($bets as $bet): ?>
                            <tr class="history__item">
                                <td class="history__name"><?= htmlspecialchars($bet['username'] ?? 'User',
                                        ENT_QUOTES | ENT_HTML5) ?></td>
                                <td class="history__price"><?= htmlspecialchars(formatted_sum($bet['total']) ?? 0,
                                        ENT_QUOTES | ENT_HTML5) ?></td>
