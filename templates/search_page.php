    ...
    <section class="lots">
        ...
        <ul class="lots__list">
            <?php if (!empty($ad_information)): ?>
                <?php foreach ($ad_information as $ad_value): ?>
                    <li class="lots__item lot">
                        <div class="lot__image">
                            <img src="../<?= htmlspecialchars($ad_value['image_url'] ?? '#') ?>" width="350"
                                 height="260"
                        ...
                        <div class="lot__info">
                            ...
                            <h3 class="lot__title"><a class="text-link"
                                                      href="/lot.php?id=<?= htmlspecialchars($ad_value['id']) ?? '' ?>"><?= htmlspecialchars($ad_value['title'] ?? 'Без названия') ?></a>
                            </h3>
                            <div class="lot__state">
                                <div class="lot__rate">
                                    <span class="lot__amount">Стартовая цена</span>
                                    <span
                                        class="lot__cost"><?= htmlspecialchars(formatted_sum($ad_value['total'] ?? 0)) ?? '' ?></span>
                                </div>
                ...
                <?php endforeach; ?>
            <?php else: ?>
                <p>По вашему запросу <b>"<?= htmlspecialchars($search); ?>"</b> нет подходящих лотов.</p>
            <?php endif; ?>
        </ul>
    </section>
